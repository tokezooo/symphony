defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server stops a turn and writes outcome marker when token budget is exceeded" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-730")
      codex_binary = Path.join(test_root, "fake-codex")
      File.mkdir_p!(workspace)
      ...
      while IFS= read -r _line; do
        ...
        case "$count" in
          1)
            ...
            ;;
          2)
            printf '%s\\n' '{"id":2,"result":{"thread":{"id":"thread-730"}}}'
            ;;
          3)
            printf '%s\\n' '{"id":3,"result":{"turn":{"id":"turn-730"}}}'
            printf '%s\\n' '{"method":"thread/tokenUsage/updated","params":{"tokenUsage":{"total":{"input_tokens":9,"output_tokens":3,"total_tokens":12}}}}'
            ;;
          *)
            sleep 1
            ;;
      ...
      issue = %Issue{
        ...
        identifier: "MT-730",
        title: "Token budget",
        description: "Stop review once budget is spent",
        state: "In Progress",
        url: "https://example.org/issues/MT-730",
        labels: ["backend"]
      }

      assert {:error, {:turn_token_budget_exceeded, %{reason: "turn_token_budget_exceeded", total_tokens: 12, token_budget: 10}}} =
               AppServer.run(workspace, "Review within budget", issue)

      outcome =
        workspace
        |> Path.join(".symphony/log/agent-outcome.json")
        |> File.read!()
        |> Jason.decode!()
      ...
      assert is_binary(outcome["written_at"])
    after
      File.rm_rf(test_root)
    end
  end
